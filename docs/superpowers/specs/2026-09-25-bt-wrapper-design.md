...
- 끝나면 결과 요약 대화상자를 띄운다. 엔진이 설치되지 않았으면 "엔진 설치" 버튼을 먼저 누르라고 안내한다.
...
- 실제 확인: 평가 때 설치한 `.dev\BallonsTranslator`에 엔진 설치 흐름을 다시 실행해 멱등성을 확인하고, 샘플 6장을 E4B로 번역
